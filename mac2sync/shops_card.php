 ...
 *   	\file       shops_card.php
 *		\ingroup    mac2sync
 *		\brief      Page to create/edit/view shops
 */
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Shops")), '', 'object_'.$object->picto);
...
{
	print load_fiche_titre($langs->trans("Shops"), '', 'object_'.$object->picto);
	...
	include DOL_DOCUMENT_ROOT.'/custom/mac2sync/core/template/edit.template.php';
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteShops'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	include DOL_DOCUMENT_ROOT.'/custom/mac2sync/core/template/view.template.php';
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook))
		{
			// TESTER CONNEXION
			if (empty($user->socid)) {
				print dolGetButtonAction('TESTER CONNEXION', '', 'default', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=check_connection');
			}
	...
	//Verification de la connexion [MAC2SYNC]
	if($action == 'check_connection'){
		include_once('PSWebServiceLibrary.php');
		try {
			//Connexion au WebService
			$webService = new PrestaShopWebservice($object->prestashop_url, $object->prestashop_ws_key, false);
			$xml = $webService->get(['resource' => 'customers']);
			echo "Connexion réussie";
		} catch (PrestaShopWebserviceException $ex) {
			echo 'Connexion échouée: <br />' . $ex->getMessage();
		}
	}
}
