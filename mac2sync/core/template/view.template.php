...
foreach ($object->fields as $key => $val) {
	...
	if ($key == 'lang') {
		...
		if (isset($val['copytoclipboard']) && $val['copytoclipboard'] == 2) {
            ...
            $tabEntity = explode(",",$object->ENTITY_CONNEXION);
            foreach($tabEntity as $ent){
                $sql = "SELECT e.label FROM `llx_entity` AS e LEFT JOIN llx_mac2sync_shops as mac2 ON mac2.ENTITY_CONNEXION = e.rowid where e.rowid = $ent";
                $res = $db->query($sql);
