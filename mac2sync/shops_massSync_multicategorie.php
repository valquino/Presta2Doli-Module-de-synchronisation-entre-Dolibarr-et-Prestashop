...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	print dol_get_fiche_head($head, 'massSyncMultiTab', $langs->trans("Shops"), -1, $object->picto);
	...
	print '<div>';
	print '<b><u>Synchroniser de catégories Dolibarr vers Prestashop:</u></b> <button onClick="document.getElementById(\'syncExistingForm\').submit()">Synchroniser <i class="fa fa-sync"></i></button>';
	print '<form action="'.$_SERVER["PHP_SELF"].'" id="syncExistingForm" method="POST">';
	print '<table border=>';
	print '<tr>';
	print '<th>Synchroniser</th>';
	print '<th>Catégorie Dolibarr</th>';
	print '<th>Catégorie Shop</th>';
	print '<th>Etat</th>';
	print '</tr>';
	...
	foreach($categories_presta->categories->category as $category){
		$field .= '<option value='.$category->id.'>';
		$field .= getPrestashopCategoryArboText($category, $prestashop_url, $prestashop_api_key);
		$field .= '</option>';
	}
