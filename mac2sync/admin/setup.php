...
/**
 * \file    mac2sync/admin/setup.php
 * \ingroup mac2sync
 * \brief   Mac2Sync setup page.
 */
...
require_once '../lib/mac2sync.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "mac2sync@mac2sync"));
...
$arrayofparameters = array(
	'MAC2SYNC_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'MAC2SYNC_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/mac2sync/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'MAC2SYNC_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'MAC2SYNC_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "Mac2SyncSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_mac2sync@mac2sync');

// Configuration header
$head = mac2syncAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "mac2sync@mac2sync");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("Mac2SyncSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'MAC2SYNC_'.strtoupper($myTmpObjectKey).'_ADDON';
