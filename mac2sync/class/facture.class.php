...
require_once DOL_DOCUMENT_ROOT.'/custom/mac2sync/class/product.class.php';
 ...
 */
class FactureMac2sync extends CommonInvoice
{
	/**
	 ...
	 */
	public function create(User $user, $notrigger = 0, $forceduedate = 0, $entity)
	{
			...
			if (!$error && empty($this->fac_rec) && count($this->lines) && is_object($this->lines[0])) {	// If this->lines is array of InvoiceLines (preferred mode)
				...
				foreach ($this->lines as $i => $val) {
					...
					if ($result >= 0) {
						...
						if ($this->type === FactureMAc2sync::TYPE_REPLACEMENT && $newinvoiceline->fk_remise_except) {
	...
	public function createFromCurrent(User $user, $invertdetail = 0)
	{
		...
		// Source invoice load
		$facture = new FactureMAc2sync($this->db);
	...
	public function createFromClone(User $user, $fromid = 0)
	{
		...
		$object = new FactureMAc2sync($this->db);
	 ...
	 */
	public function createFromOrder($object, User $user,$entity)
	{
		global $conf, $hookmanager;
		...
		for ($i = 0; $i < $num; $i++) {
			$line = new FactureLigneMAc2sync($this->db);
		...
		$ret = $this->create($user,0,0,$entity);
	...
	public function fetchPreviousNextSituationInvoice()
	{
		...
		if ($result && $this->db->num_rows($result) > 0) {
			while ($objp = $this->db->fetch_object($result)) {
				$invoice = new FactureMAc2sync($this->db);
	...
	public function insert_discount($idremise)
	{
		...
		if ($result > 0) {
			...
			$facligne = new FactureLigneMAc2sync($this->db);
			...
			if ($remise->fk_facture_source > 0) {
				$srcinvoice = new FactureMAc2sync($this->db);
	...
	public function validate($user, $force_number = '', $idwarehouse = 0, $notrigger = 0, $batch_rule = 0)
	{
		...
		if ($this->type == self::TYPE_REPLACEMENT) {		// if this is a replacement invoice
			...
			$facreplaced = new FactureMAc2sync($this->db);
			...
			if ($idreplacement && $idreplacement != $this->id) {
				$facreplacement = new FactureMAc2sync($this->db);
	...
	public function addline(
		...
		if ($this->statut == self::STATUS_DRAFT) {
			...
			// Insert line
			$this->line = new FactureLigneMAc2sync($this->db);
	 ...
	 *
	 * @param  FactureLigneMAc2sync $line       Invoice line
	...
	public function load_board($user)
	{
		...
		if ($resql) {
			...
			$generic_facture = new FactureMAc2sync($this->db);
	...
	public function get_prev_sits()
	{
		...
		if ($resql && $resql->num_rows > 0) {
			while ($row = $this->db->fetch_object($resql)) {
				$id = $row->rowid;
				$situation = new FactureMAc2sync($this->db);
				$situation->fetch($id);
				$res[] = $situation;
			}
		} else {
	...
	public function hasDelay()
	{
		...
		if ($this->statut != FactureMAc2sync::STATUS_VALIDATED) {
			return false;
		}
	...
	public function displayRetainedWarranty()
	{
		...
		if (!empty($this->retained_warranty)) {
			...
			if ($this->type == FactureMAc2sync::TYPE_SITUATION && !empty($conf->global->INVOICE_RETAINED_WARRANTY_LIMITED_TO_FINAL_SITUATION)) {
				// Check if this situation invoice is 100% for real
				$displayWarranty = false;
				if (!empty($this->situation_final)) {
					$displayWarranty = true;
				} elseif (!empty($this->lines) && $this->status == FactureMAc2sync::STATUS_DRAFT) {
	...
	public function sendEmailsRemindersOnInvoiceDueDate($nbdays = 0, $paymentmode = 'all', $template = '')
	{
		...
		*/
		...
		$tmpinvoice = new FactureMAc2sync($this->db);
 ...
 */
class FactureLigneMac2sync extends CommonInvoiceLine
{
	/**
	 * @var string ID to identify managed object
	 */
	...
	public function insert($notrigger = 0, $noerrorifdiscountalreadylinked = 0)
	{
		...
		if (!empty($this->fk_product) && $this->fk_product > 0) {
			...
			$result = ProductMac2sync::isExistingObject('product', $this->fk_product);
			if ($result <= 0) {
	...
	public function get_prev_progress($invoiceid, $include_credit_note = true)
	{
		...
		if (is_null($this->fk_prev_id) || empty($this->fk_prev_id) || $this->fk_prev_id == "") {
			...
			if (!isset($invoicecache[$invoiceid])) {
				$invoicecache[$invoiceid] = new FactureMac2sync($this->db);
				$invoicecache[$invoiceid]->fetch($invoiceid);
			}
			if ($invoicecache[$invoiceid]->type != FactureMac2sync::TYPE_SITUATION) {
				return 0;
			}
			...
			if ($resql && $resql->num_rows > 0) {
				...
				if ($include_credit_note) {
					...
					$sql .= " AND f.type = ".FactureMac2sync::TYPE_CREDIT_NOTE;
