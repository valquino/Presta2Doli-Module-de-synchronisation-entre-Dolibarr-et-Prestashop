...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	print dol_get_fiche_head($head, 'logs', $langs->trans("Shops"), -1, $object->picto);
	...
	if($action == 'clean'){
		$ecrire = fopen('core/modules/mac2sync_log.txt',"w");
		ftruncate($ecrire,0);
	}
	print '<div>';
	print '<h1>Logs</h1><br>';
	print '<a href="core/modules/mac2sync_log.txt" download><button>Telecharger les logs</button></a>&nbsp;';
	print '<a href="shops_logs.php?action=clean"><button>Purger les logs</button></a><br>';
	
	print '<textarea rows="50" cols="150">';
	print file_get_contents("core/modules/mac2sync_log.txt");
	print '</textarea>';
	print '<br><br>';
