...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	print dol_get_fiche_head($head, 'massSyncMultiTabPresta', $langs->trans("Shops"), -1, $object->picto);
	...
	print '<div>';
	print '<h1>En cours de développement</h1>';
	print '<form action="'.$_SERVER["PHP_SELF"].'" id="syncExistingForm" method="POST">';
	print '<b><u>Synchroniser les produits de Prestashop vers Dolibarr:</u></b> <button onClick="document.getElementById(\'syncExistingForm\').submit()">Synchroniser <i class="fa fa-sync"></i></button>';
	print '<table border=>';
	print '<tr>';
	print '<th>Synchroniser</th>';
	print '<th>Catégorie Prestashop</th>';
	print '<th>Catégorie Dolibarr</th>';
	print '<th>Etat</th>';
	print '</tr>';
	...
	foreach($categories as $categorie){
		print '<tr>';
		print '<td><input type=checkbox name="sync'.$categorie['id'].'"></td>';
		
		print '<td>';
		print '<select name=presta_cate_id'.$categorie['id'].'>';
		foreach($categories_presta->categories->category as $category){
			print '<option value='.$category->id.'>';
			//print $category->name->language;
		...
		print '</td>';
		print '<td>';
		print '<i class="fa" id="icon'.$categorie['id'].'"></i>';
		print '</td>';
		print '</tr>';
	}
	...
	print '<input type=hidden value="SYNC_EXISTING_CATES" name="action">';
	print '</table>';
	print '</form>';
	print '</div>';
	...
	//Synchronisation catégories existantes
	if($action == 'SYNC_EXISTING_CATES'){
		echo "<b><u>Produits synchronisés: </b></u><br>";
		global $db;
		
		  foreach($categories as $categorie){
			  $cate_id = $categorie['id'];
			  if(isset($_POST['sync'.$cate_id]) && $_POST['sync'.$cate_id] == "on"){
				  print '<script type="text/javascript">document.getElementById("icon'.$cate_id.'").classList.add("fa-check");</script>';
				  $doli_cate_id = $cate_id;
				  $prestashop_category_id = $_POST['presta_cate_id' .$cate_id];
				  $object->syncToExistingCategoryFromPrestaToDoli($prestashop_category_id, $doli_cate_id, $prestashop_api_key, $prestashop_url);
			  }
		  }
